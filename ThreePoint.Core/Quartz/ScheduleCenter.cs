...
namespace ThreePoint.Core.Quartz
{
    /// <summary>
    /// 任务调度中心
    /// </summary>
    public class ScheduleCenter : IScheduleCenter
    {
            ...
            string Description = null)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(JobName) || string.IsNullOrEmpty(JobGroup) || string.IsNullOrEmpty(JobNamespaceAndClassName) || string.IsNullOrEmpty(CronExpress))
                {
                    result.ResultCode = -3;
                ...
                //判断开始时间,非特殊情况，一般不允许出现空时间值
                //if (StarTime.HasValue && StarTime < DateTime.Now) StarTime = DateTime.Now;
                StarTime ??= DateTime.Now; //判断是否输入开始时间
                DateTimeOffset starRunTime = DateBuilder.NextGivenSecondDate(StarTime, 1);
                //EndTime为空时设为永远执行
                EndTime ??= StarTime.Value.AddDays(-1);
                ...
                Type jobType;
                if (!string.IsNullOrEmpty(JobAssemblyName))
                {
                    //通过反射
                    //读取程序集路径 type.Assembly.Location @"E:\Program2013\20160416_Reflect\DrHao\bin\Debug\DrHao.dll"
                    Assembly assembly = Assembly.LoadFile(JobAssemblyName);
                    ////读取类型  JobNamespaceAndClassName = namespace.class
                    jobType = assembly.GetType(JobNamespaceAndClassName);
                }
                else
                {
                    //通过引用
                    jobType = Type.GetType(JobNamespaceAndClassName, false, true);
                }
