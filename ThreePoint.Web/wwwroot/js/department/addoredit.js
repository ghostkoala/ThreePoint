...
$("#btnSearch").click(function () {
    ...
    if (searchDepartment == "" || searchDepartment == null) {
        $("#departmentResult").val("请输入要搜索的部门名称");
        return;
    }
    $.ajax({
        type: "get",
        url: "/Department/GetSearchNameForAreaText",
        data: { keyWord: searchDepartment },
        success: function (data, status) {
            $("#departmentResult").val(data);
        },
        error: function () {
            $("#departmentResult").val("搜索失败！");
        },
        complete: function () {
